...
void Comms_Init(void) {
	...
	GPIOA->MODER = GPIOA->MODER & 0xFCFFFFFF | 0x02000000; 
	GPIOB->AFR[0] = GPIOB->AFR[0] & 0x00FFFFFF; 
	GPIOB->MODER = GPIOB->MODER & 0xFFFF0FFF | 0x0000A000; 
	// USART1 with DE, baud 115200
	RCC->APBENR2 |= RCC_APBENR2_USART1EN; 
	USART1->CR1 = 0x20210020; 
...
static int Comms_InternalHandleExec(void) {
	...
	if(buflen == 0) return 0; // Invalid if length = 0
	int argindex = 0; 
	int bufindex = 0; 
	// 1. Skip leading spaces
	for(; bufindex < Comms_InternalIndex; bufindex++) {
		if(Comms_CommandBuf[bufindex] != ' ') break; 
	}
	for(; argindex < COMMS_MAX_CMD_ARGS; argindex++) {
		...
		if(length == 0) break; // We have no more
	...
	if(argindex == 0) return 0; // Invalid if no valid args
	Comms_Argc = argindex; 
	return 1; // Valid
}
...
void Comms_BlockingTxString(const char * str) {
	for(int i = 0; i < 100; i++) { // Max TX length to limit array out of bounds
		char ch = str[i]; 
		if(ch == 0x0) return; 
...
int Comms_ArgCompare(int argindex, const char * str) {
	if(argindex >= Comms_Argc) return 0; 
	int length = 0; 
	for(; length < 100; length++) {
		if(!str[length]) break; 
	}
	if(length != Comms_Argv[argindex].length) return 0; 
	int offset = Comms_Argv[argindex].offset; 
	for(int i = 0; i < length; i++) {
		if(str[i] != Comms_CommandBuf[offset + i]) return 0; 
	}
	return 1; 
}

int Comms_ArgScanInt(int argindex, int * val) {
	if(argindex >= Comms_Argc) return -1; // Error
